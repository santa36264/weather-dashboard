    ...
    <main class="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6 pb-24 space-y-4 sm:space-y-5">
      ...
      <div v-else-if="!weather" class="flex flex-col items-center justify-center py-24 text-center px-4">
        <div class="relative mb-6">
          <div class="w-24 h-24 rounded-full bg-gradient-to-br from-primary-100 to-secondary-100 dark:from-primary-900/30 dark:to-secondary-900/30 flex items-center justify-center">
        ...
        <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-xs text-sm">
          Allow location access or search for a city manually.
        </p>
        <button
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
          ...
          <!-- Side panel — stacks below on mobile, beside on desktop -->
...
import { CloudSun, LocateFixed, RefreshCw, AlertCircle, Sunrise, Sunset, Navigation, Wind } from 'lucide-vue-next'
...
// Auto-detect location on page load if no city is persisted
onMounted(async () => {
  if (store.selectedCity) {
    // Re-fetch fresh data for persisted city
